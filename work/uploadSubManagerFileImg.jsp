...
try{	
	...
	if(m_num != null && !m_num.equals("")){
		try{
			...
			if(admin.countManagerAnswer("m_num='" + info.m_num + "'") > 0){
				admin.modifiedManagerAnswer(m_num, m_msg, "/file/" + rank + "/answer/img/thum_" + file_name, "/file/" + rank + "/answer/img/" + file_name, manager_name, manager_phone, r_date);
			}else{
				admin.insertManagerAnswer(myModel);
				admin.updateManagerReport(m_num);
			}
...
$(document).ready(function(){
	window.setTimeout("opener.location = '<%=pathtalkfile.getTalkURLPath()%>/manager/formSiseolReport.jsp'", 1);
	window.close();
});
</script>
<body>
