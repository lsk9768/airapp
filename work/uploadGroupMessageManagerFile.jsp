...
int stateThumNail = 0;
 ...
 try {
	...
	try {	
		...
		String date = dateFormat.format(dateSecond) + String.format("%03d", microSeconds % 1000);
		
		String sms_id = "sms"+date;
			...
			if(file_count+2000<=m){
					...
					try {
					  ...
					  if(type.equals("img")){
											...
											FileOutputStream fosThum = new FileOutputStream(path+"/file/group/"+groupid+"/"+type+"/"+tmpFileName);
						...
						msg = tmpFileName;	
		   				msg = vctDNSList.elementAt(dns_count).name + "/file/group/"+groupid+"/"+type+"/"+tmpFileName;	   
						if(dns_count+1>=dns_maxcount) dns_count = 0;
else dns_count++;
				...
				if(info.state.equals("GROUP")){
					...
					if(n==0){
									...
									try {
									   ...
									   FileOutputStream fos = new FileOutputStream(filepath.getFilePath()+"/file/group/"+info.name+"/"+type+"/"+tmpFileName1);
						...
						if(file_count2+2000<=l){
							...
							try {
							   ...
							   fos.close();
								...
								msg = tmpFileName;	
								//file_name = "/file/group/"+info.name+"/"+type+"/"+file_name;	
								msg = vctDNSList.elementAt(dns_count).name + "/file/group/"+info.name+"/"+type+"/"+tmpFileName;	   
