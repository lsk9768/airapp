	...
	while (iter.hasNext()) {
		...
		if (item.isFormField()) { // 올린 사람 이름이면
			...
			case 6:{
				...
				System.out.println("SendType="+TextMsg);
				break;
			}
			...
			for(int n=0; n<vctMember.size(); n++){
				...
				if(info.state.equals("GROUP")){
					...
					if(m==0&&n==0){
							...
							if(admin.checkSMSMessage(sms_id)<=0)admin.insertSMSMessage(group_id,userid,"broadcast1",msg,sms_id);
							
							if(admin.checkSMSMessage(sms_id)<=0)admin.insertSMSMessage(group_id,userid,"broadcast1",msg,sms_file_id);
