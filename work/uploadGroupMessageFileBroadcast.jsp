...
String MsgType = "2";      //발송업무 0:일반 1:경보
	...
	while (iter.hasNext()) {
		...
		if (item.isFormField()) { // 올린 사람 이름이면
			...
			case 5:{
				...
				for(int i=0; i<msg.length(); i++){
					if(msg.charAt(i)==CheckString1.charAt(0)) tmpMSG+="\\";
					if(msg.charAt(i)==CheckString2.charAt(0)) tmpMSG+="\\'";
					else tmpMSG+=msg.charAt(i);
				}
		...
		String fileFieldName = item.getName();
			...
			if(fileName==""){
				...
				File file = new File(path + "/file/group/"+groupid+"/media", fileName);
				//올리려는 파일과 같은 이름이 존재하면 중복파일 처리
				 if ( file.exists() ){
					for(int k=0; true; k++){
						 String tempFile = k+"_"+fileName;
						 //파일명 중복을 피하기 위한 일련 번호를 생성하여 파일명으로 조합
						 file = new File(path+"/file/group/"+groupid+"/media", tempFile);
			...
			item.write(file);
		   	...
		   	file_name = "/file/group/"+groupid+"/media/"+fileName;
		...
		for(int m=0; m<maxcount; m=m+50){
			...
			if(m>=50){
					...
					try {
						...
						File file = new File(path+"/file/group/"+groupid+"/media", tmpFileName);
						if ( file.exists() ){
							 for(int p=1; true; p++){
					   ...
					   FileInputStream fis = new FileInputStream(path+"/file/group/"+groupid+"/media/"+fileName);
					   FileOutputStream fos = new FileOutputStream(path+"/file/group/"+groupid+"/media/"+tmpFileName);
					   ...
					   fos.close();
						...
						file_name = tmpFileName;	
		   				file_name = "/file/group/"+groupid+"/media/"+file_name;	   
					...
					if(n==0){
						...
						File dirMain2 = new File(filepath.getFilePath()+"/file/group/"+info.name);
									...
									try {
										...
										File file = new File(path+"/file/group/"+info.name+"/media", tmpFileName1);
										if ( file.exists() ){
											 for(int p=1; true; p++){
												 String tempFile = p+"_"+tmpFileName1;
												 //파일명 중복을 피하기 위한 일련 번호를 생성하여 파일명으로 조합
												 file = new File(path+"/file/group/"+info.name+"/media", tempFile);
												if(!file.exists()){ //존재하지 않는 경우
									   ...
									   FileOutputStream fos = new FileOutputStream(filepath.getFilePath()+"/file/group/"+info.name+"/media/"+tmpFileName1);
					...
					for(int l=0; l<GroupMaxCount; l=l+50){
						...
						if(l>=1){
							...
							try {
								...
								File file = new File(path+"/file/group/"+info.name+"/media", tmpFileName);
								if ( file.exists() ){
							   ...
							   FileInputStream fis = new FileInputStream(path+"/file/group/"+info.name+"/media/"+fileName);
							   FileOutputStream fos = new FileOutputStream(path+"/file/group/"+info.name+"/media/"+tmpFileName);
							   ...
							   fos.close();
								...
								file_name = tmpFileName;	
								file_name = "/file/group/"+info.name+"/media/"+file_name;	
						...
						if(l==0){
							...
							if(admin.checkSMS(sms_id)<=0){
								...
								if(admin.checkSMSMessage(sms_id)<=0)admin.insertSMSMessage(group_id,userid,"broadcast1",msg+"|||"+file_name,sms_id);
