...
String MsgType = "2";      //발송업무 0:일반 1:경보 2:방송
 ...
 try {
	...
	while (iter.hasNext()) {
		...
		if (item.isFormField()) { // 올린 사람 이름이면
			...
			*/
		    //out.print(i + " : " + item.getString("UTF-8"));
			//System.out.println(i+":"+item.getString("UTF-8"));
			i++;
		...
		String fileFieldName = item.getName();
			...
			if(fileName==""){
				...
				File file = new File(path + "/file/group/"+groupid+"/broadcast", fileName);
				//올리려는 파일과 같은 이름이 존재하면 중복파일 처리
				 if ( file.exists() ){
					for(int k=0; true; k++){
						 String tempFile = k+"_"+fileName;
						 //파일명 중복을 피하기 위한 일련 번호를 생성하여 파일명으로 조합
						 file = new File(path+"/file/group/"+groupid+"/broadcast", tempFile);
			...
			for(int m=0; m<file_name.length(); m++){
				if(file_name.charAt(m)==CheckString1.charAt(0)) tmpMSG+="\\";
				if(file_name.charAt(m)==CheckString2.charAt(0)) tmpMSG+="\\'";
				else tmpMSG+=file_name.charAt(m);
			} 
		...
		for(int m=0; m<maxcount; m=m+50){
			...
			if(file_count+200<=m){
					...
					try {
						...
						File file = new File(path+"/file/group/"+groupid+"/broadcast", tmpFileName);
						if ( file.exists() ){
							 for(int p=1; true; p++){
					   ...
					   FileInputStream fis = new FileInputStream(path+"/file/group/"+groupid+"/broadcast/"+fileName);
					   FileOutputStream fos = new FileOutputStream(path+"/file/group/"+groupid+"/broadcast/"+tmpFileName);
				...
				if(info.state.equals("GROUP")){
					...
					if(n==0){
									...
									try {
										...
										File file = new File(path+"/file/group/"+info.name+"/broadcast", tmpFileName1);
										if ( file.exists() ){
											 for(int p=1; true; p++){
												 String tempFile = p+"_"+tmpFileName1;
												 //파일명 중복을 피하기 위한 일련 번호를 생성하여 파일명으로 조합
												 file = new File(path+"/file/group/"+info.name+"/broadcast", tempFile);
												if(!file.exists()){ //존재하지 않는 경우
					...
					for(int l=0; l<GroupMaxCount; l=l+50){
						...
						if(file_count2+200<=l){
							...
							try {
								...
								File file = new File(path+"/file/group/"+info.name+"/broadcast", tmpFileName);
								if ( file.exists() ){
							   ...
							   FileInputStream fis = new FileInputStream(path+"/file/group/"+info.name+"/broadcast/"+fileName);
							   FileOutputStream fos = new FileOutputStream(path+"/file/group/"+info.name+"/broadcast/"+tmpFileName);
						...
						if(l==0){
							...
							if(admin.checkSMS(sms_id)<=0){
								...
								if(admin.checkSMSMessage(sms_id)<=0)admin.insertSMSMessage(group_id,userid,"broadcast3",msg+"|||"+file_name,sms_id);
						...
						for(int k=0; k<vctGroupMember.size(); k++){
							...
							infoSMS.send_msg_type = "broadcast3";	
          ...
          <script type="text/javascript">
		  	alert('음성 파일은 50MB 이상 업로드 할 수 없습니다.');
			window.setTimeout("location='<%=pathtalkfile.getTalkURLPath()%>/admin/formGroupBroadcast.jsp'",1); //1ms후 페이지 자동이동
			</script>
          <%
		 //out.print("2"); 
