...
String MsgType = "0";      //발송업무 0:일반 1:경보
String SendType = "1";     //발송구분 0:Talk+SMS 1:Talk 2:SMS
 ...
 try {
	...
	while (iter.hasNext()) {
		...
		if (item.isFormField()) { // 올린 사람 이름이면
			...
			case 3:{
				multi_type = item.getString("UTF-8");
				break;
			}
			case 4:{
				Title = item.getString("UTF-8");
				
				if(Title==null) Title="멀티메시지";
				else{
					if(Title.equals(""))Title="멀티메시지";
					else{
						Title = strReplace.replaces(Title);
					}
				}
			...
			case 5:{
				...
				TextMsg = strReplace.replaces(TextMsg);
				
				break;
			}
			case 6:{
				Btn_Title_1 = item.getString("UTF-8");
				break;
			}
			case 7:{
				Btn_Url_1 = item.getString("UTF-8");
				
				Btn_Url_1 = strReplace.replaces(Btn_Url_1);
				
				break;
			}
			
			case 8:{
				Btn_Title_2 = item.getString("UTF-8");
				break;
			}
			case 9:{
				Btn_Url_2 = item.getString("UTF-8");
				Btn_Url_2 = strReplace.replaces(Btn_Url_2);
				break;
			}
		...
		String fileFieldName = item.getName();
			...
			if(type.equals("img")){
				...
				image.ImageScaling(path + "/file/group/"+groupid+"/"+type,file_name,600,600);
			}
			String CheckString1 = "\\";
			String CheckString2 = "'";
		
		    SendFileName = "/file/group/"+groupid+"/"+type+"/"+file_name;
		
			String tmpMSG = "";
		
			
			for(int m=0; m<SendFileName.length(); m++){
				if(SendFileName.charAt(m)==CheckString1.charAt(0)) tmpMSG+="\\";
				if(SendFileName.charAt(m)==CheckString2.charAt(0)) tmpMSG+="\\'";
				else tmpMSG+=SendFileName.charAt(m);
			} 
					...
					for(int l=0; l<GroupMaxCount; l=l+50){
						...
						if(l==0){
							...
							if(admin.checkSMS(sms_id)<=0){
								...
								if(admin.checkSMSMessage(sms_id)<=0)admin.insertSMSMessage(group_id,userid,"multi",Title,sms_id);
						...
						for(int k=0; k<vctGroupMember.size(); k++){
							...
							CreateMultiMessage multi_message = new CreateMultiMessage();
							
							
							String msg = multi_message.CreateData(multi_type, Title, TextMsg, SendFileName, type, Btn_Title_1, Btn_Url_1, Btn_Title_2, Btn_Url_2);
...
<script type="text/javascript">		 
	...
	window.setTimeout("location='<%=pathtalkfile.getTalkURLPath()%>/admin/formGroupMessageMultiSelect.jsp'",1); //1ms후 페이지 자동이동
</script>
